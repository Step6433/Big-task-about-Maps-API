    ...
    if response:
        ...
        try:
            ...
            if flag:
                ...
                post_code = '0'
            return [list(map(float, toponym_coords.split())), adres, post_code]
...
post_rect = pygame.Rect(5, 500, 390, 30)
...
while running:
    ...
    if m:
        ...
        if flag_post:
            pygame.draw.rect(screen, (248, 96, 74), post_rect)
            text_post = small_font.render('Выключить дописывание почтового индекса', True, (0, 0, 0))
        else:
            pygame.draw.rect(screen, (255, 204, 0), post_rect)
            text_post = small_font.render('Включить дописывание почтового индекса', True, (0, 0, 0))
        screen.blit(text_post, (post_rect.x + 5, post_rect.y + 5))
        pygame.display.flip()
